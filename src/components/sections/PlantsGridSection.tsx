...
import PlantHero3D from "@/components/plant/PlantHero3D";

import { getAllPlantsLatestOEE } from "@/services/oeeService";

export default function PlantsGridSection() {
  ...
  return (
    <section id="plants" className="relative border-t border-[var(--industrial-line)] bg-[var(--industrial-bg)] px-6 py-28 lg:px-10">
      <div className="mx-auto max-w-[1400px]">
        <p className="font-mono-industrial text-xs tracking-[0.35em] text-[var(--accent-cyan)]">ENTER THE PLANTS</p>
          ...
          {PLANTS.map((plant) => {
            ...
            return (
              ...
              >
                ...
                />
                <p className="font-mono-industrial text-[10px] font-bold tracking-[0.25em] text-[var(--steel)] relative z-10">
                ...
                <p className="mt-4 text-sm text-[var(--steel)] relative z-10">{plant.description}</p>

                <div className="mt-8 flex items-end justify-between">
                  <div>
                    <p className="font-mono-industrial text-[9px] tracking-[0.2em] text-[var(--steel)]">LATEST OEE</p>
                    <p className="font-display text-4xl font-semibold" style={{ color }}>
                      {oee > 0 ? oee.toFixed(1) : "—"}
                      <span className="text-lg text-[var(--steel)]">%</span>
                    </p>
                  </div>
                  <span className="font-mono-industrial text-[10px] text-[var(--steel)] transition-colors group-hover:text-[var(--steel-light)]">
                    OPEN DASHBOARD →
                  </span>
                </div>
              </Link>
